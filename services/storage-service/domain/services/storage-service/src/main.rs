...
use storage_domain::{BatchWriter, BatchConfig};
...
async fn main() -> Result<()> {
    ...
    // 创建适配器
    let clickhouse_adapter = ClickHouseAdapter::new(config.clickhouse_url.clone());
    info!("✅ ClickHouse适配器创建成功");

    // 创建BatchWriter
    let batch_writer = BatchWriter::new(clickhouse_adapter);

    // 创建用例
    let store_use_case = Arc::new(tokio::sync::Mutex::new(
        StoreQuoteUseCase::new(batch_writer)
    ));
