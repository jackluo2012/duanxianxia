...
async fn test_is_trading_day_weekday() {
    ...
    let result = calendar
        .is_trading_day(NaiveDate::from_ymd_opt(2026, 1, 2).unwrap())
        .await;
...
async fn test_is_trading_day_weekend() {
    ...
    let result = calendar
        .is_trading_day(NaiveDate::from_ymd_opt(2026, 1, 4).unwrap())
        .await;
...
async fn test_get_current_status_on_trading_day() {
    ...
    let now = Local::now();
    assert!(
        status.next_open_time > now,
        "next_open_time should be in the future"
    );
...
async fn test_trading_day_boundary_cases() {
    ...
    assert_eq!(
        is_trading, false,
        "Jan 31, 2026 is Saturday, should not be trading day"
    );
    ...
    assert_eq!(
        is_trading, true,
        "Dec 31, 2026 is Thursday, should be trading day"
    );
    ...
    assert_eq!(
        is_trading, true,
        "Feb 29, 2024 is Thursday, should be trading day"
    );
}
